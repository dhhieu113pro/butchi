...
pub fn append(action: TextAction, source: &str, result: &str, message: &str, target_language: Option<&str>) {
    let cfg = config::load();
    if cfg.history_retention_days == 0 { return; }
    let action_name = match action { TextAction::Translate => "translate", TextAction::Rewrite => "rewrite" };
    let ts = now_ms();
    let entry = HistoryEntry {
        id: format!("{ts}-{action_name}"),
        ts,
        action: action_name.into(),
        source: core_logic::truncate_text(source, 8000),
        result: core_logic::truncate_text(result, 16000),
        message: core_logic::truncate_text(message, 1000),
        target_language: target_language.map(|s| core_logic::truncate_text(s, 100)),
    };
    ...
    if let Ok(conn) = open_db() {
        ...
        let _ = conn.execute(
            "INSERT OR REPLACE INTO history (id, ts, action, source, result, message, target_language)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
            params![entry.id, entry.ts as i64, entry.action, entry.source, entry.result, entry.message, entry.target_language],
        );
    }
}

pub fn search(query: Option<&str>, action: Option<&str>, limit: Option<usize>) -> Result<Vec<HistoryEntry>, String> {
    let conn = open_db()?;
    let cfg = config::load();
    ...
    let rows = stmt.query_map(params![q, pattern, action, lim], |row| {
        Ok(HistoryEntry {
            id: row.get(0)?, ts: row.get::<_, i64>(1)? as u64, action: row.get(2)?,
            source: row.get(3)?, result: row.get(4)?, message: row.get(5)?, target_language: row.get(6)?,
        })
    }).map_err(|e| format!("search history: {e}"))?;
    rows.collect::<Result<Vec<_>, _>>().map_err(|e| format!("read history: {e}"))
}
...
pub fn delete(id: &str) -> Result<(), String> {
    let conn = open_db()?;
    conn.execute("DELETE FROM history WHERE id = ?1", params![id]).map_err(|e| format!("delete history entry: {e}"))?;
    Ok(())
}

pub fn clear() -> Result<(), String> {
    let conn = open_db()?;
    conn.execute("DELETE FROM history", []).map_err(|e| format!("clear history: {e}"))?;
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn retention_cleanup_handles_forever_zero_and_positive() {
        let conn = Connection::open_in_memory().unwrap();
        conn.execute_batch("CREATE TABLE history (id TEXT PRIMARY KEY, ts INTEGER NOT NULL, action TEXT NOT NULL, source TEXT NOT NULL, result TEXT NOT NULL, message TEXT NOT NULL, target_language TEXT NULL);").unwrap();
        conn.execute("INSERT INTO history VALUES ('old', 1, 'rewrite', 'a', 'b', 'c', NULL)", []).unwrap();
        assert!(cleanup_retention(&conn, -1).is_ok());
        assert_eq!(conn.query_row("SELECT count(*) FROM history", [], |r| r.get::<_, i64>(0)).unwrap(), 1);
        assert!(cleanup_retention(&conn, 30).is_ok());
        assert_eq!(conn.query_row("SELECT count(*) FROM history", [], |r| r.get::<_, i64>(0)).unwrap(), 0);
        conn.execute("INSERT INTO history VALUES ('x', 1, 'rewrite', 'a', 'b', 'c', NULL)", []).unwrap();
        assert!(cleanup_retention(&conn, 0).is_ok());
        assert_eq!(conn.query_row("SELECT count(*) FROM history", [], |r| r.get::<_, i64>(0)).unwrap(), 0);
    }
}
