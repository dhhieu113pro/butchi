...
mod tests {
    ...
    #[test] fn now_is_plausible() { assert!(now_ms() > 1_700_000_000_000); }
    #[test] fn sqlite_search_filters_orders_and_limits() { let conn = memory_db(); insert(&conn, &entry("1", 10, "rewrite", "Hello World", "Fixed text")); insert(&conn, &entry("2", 30, "translate", "Good morning", "Xin chao")); insert(&conn, &entry("3", 20, "translate", "HELLO again", "Xin chao lan nua")); let all = search_conn(&conn, None, None, None).unwrap(); assert_eq!(all.iter().map(|e| e.id.as_str()).collect::<Vec<_>>(), vec!["2", "3", "1"]); assert_eq!(search_conn(&conn, Some("hello"), None, None).unwrap().len(), 2); assert_eq!(search_conn(&conn, Some("xin CHAO"), Some(" TRANSLATE "), None).unwrap().len(), 2); assert_eq!(search_conn(&conn, None, None, Some(1)).unwrap().len(), 1); assert_eq!(search_conn(&conn, None, None, Some(0)).unwrap().len(), 1); }
    #[test] fn retention_covers_forever_disabled_and_day_cutoff() { let conn = memory_db(); insert(&conn, &entry("old", 1_000, "rewrite", "a", "b")); insert(&conn, &entry("new", 200_000_000, "rewrite", "a", "b")); cleanup_retention_at(&conn, -1, 200_000_000).unwrap(); assert_eq!(search_conn(&conn, None, None, None).unwrap().len(), 2); cleanup_retention_at(&conn, 1, 200_000_000).unwrap(); assert_eq!(search_conn(&conn, None, None, None).unwrap()[0].id, "new"); cleanup_retention_at(&conn, 0, 200_000_000).unwrap(); assert!(search_conn(&conn, None, None, None).unwrap().is_empty()); }
