...
if (-not (Test-Path $ApplicationPath)) {
    throw "Candidate application not found: $ApplicationPath"
}
...
try {
    ...
    if (-not $ready) {
        throw "Candidate application did not become input-idle within $StartupTimeoutSeconds seconds."
    }
...
if ($inferenceExitCode -ne 0) {
    throw "Candidate inference command failed with exit code ${inferenceExitCode}."
}

$telemetryText = ($rawInference | Out-String).Trim()
if ([string]::IsNullOrWhiteSpace($telemetryText)) {
    throw 'Candidate inference command produced no telemetry JSON.'
}
...
foreach ($name in @('popoverToDispatch', 'firstToken', 'tokensPerSecond', 'vram')) {
    if ($null -eq $telemetry.$name) {
        throw "Candidate inference telemetry is missing '$name': $telemetryText"
    }
}
